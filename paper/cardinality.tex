...
than annotating them, which makes $k$ exact and the gold set free. We report a
slope rather than an existence claim. We show the effect is present under
unconstrained decoding and absent under a schema, inverting the expectation that
grammars cost accuracy. And we state the constraint that any study on this axis
inherits: count, density and length are mechanically linked, so fixing two
...
constrained decoding as hugely superior at exactly the level that decides the
study. That arm was discarded rather than repaired. In the reported runs, zero
...
The grammar is not the cost. On the axis of how many items a model must emit,
schema-constrained decoding is the only thing holding recall flat, and removing
it produces a decline that clears a pre-registered threshold. The mechanism is a
...
exactly the long-list cases where extraction is most valuable, and saves nothing.
